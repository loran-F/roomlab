...
function setup(s){var signature;do{var digits=s.round===0?'1234':Array.from({length:10},function(_,i){return String(i);}).sort(function(){return Math.random()-.5;}).slice(0,4).join('');s.picture=digits;s.rows=Array.from({length:s.round===0?6:8},function(_,i){return {owner:i%2?'B':'A',offset:(Math.random()<.5?-1:1)*(4+Math.floor(Math.random()*8))*STEP};});signature=digits+':'+s.rows.map(function(r){return r.offset;}).join(',');}while(signature===previousPuzzle[s.round]||['A','B'].some(function(w){return new Set(s.rows.filter(function(r){return r.owner===w;}).map(function(r){return r.offset;})).size<2;}));previousPuzzle[s.round]=signature;try{sessionStorage.setItem('roomlab-password-recent-v2',JSON.stringify(previousPuzzle));}catch(_){}s.confirmed={A:false,B:false};s.ready={A:false,B:false};}
function create(ctx){var start=ctx&&!ctx.tutorial?1:0,s={startRound:start,endRound:ctx?start:1,id:uid(),round:start,phase:'ready',win:false,remaining:start?4:5,maxMistakes:start?4:5,seq:{A:0,B:0},again:{A:false,B:false},notice:''};setup(s);return s;}
...
function publish(){var q=session;if(!q||q.failed||!q.state||(q.ctx&&!q.ctx.isActive()))return;if(!q.ctx&&q.state.phase==='done'&&q.state.again.A&&q.state.again.B){q.state=create();q.seq=0;selected=0;}view=snapshot(q.state,'A');draw();packet({t:q.ctx?'psRoomState':'psState',proto:PROTO,roomSession:q.ctx?q.ctx.sessionId:null,match:q.match,frame:++q.frame,state:snapshot(q.state,'B')});if(q.ctx&&q.state.phase==='done'&&!q.settled){q.settled=true;var captured=q.ctx;q.finishTimer=setTimeout(function(){if(session===q&&!q.failed&&captured.isActive())captured.finish(q.state.win);},700);}}
...
function draw(){var q=session;if(!q||q.failed||!view)return;var s=view,w=q.role,body=document.getElementById('ps-body');if(!body)return;var structure=s.id+':'+s.round+':'+s.phase;
...
function lobby(w){clearTimers();closePeer();clearNetZones();S.mod=MODE;S.role=w;window._dungeon=false;window._netMode=w==='A'?'host':'guest';selected=0;drawKey='';scaffold(w);
 ...
 c.on('data',function(d){if(session!==q||q.failed||!d)return;q.lastSeen=Date.now();
  ...
  else if(w==='B'&&d.t==='psState'&&d.proto===PROTO&&d.state&&d.match===q.match&&Number.isSafeInteger(d.frame)&&d.frame>q.lastFrame){q.lastFrame=d.frame;if(view&&view.id!==d.state.id){q.seq=0;selected=0;}view=d.state;draw();}
 });
