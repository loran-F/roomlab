...
function draw(s,who){if(!$('bm-canvas'))return;view=s;$('count').textContent=Math.ceil(s.time)+'s';$('goal').textContent='入库 '+s.got+' / '+s.goal;$('bm-misses').textContent='失误 '+s.spills+' / '+s.limit;
 ...
 var current=s.bricks[0];$('msg').textContent=s.rescue?s.msg+'（'+s.rescue.left.toFixed(1)+'秒）':current&&current.stress>.15?'倾斜太大！马上放平，玻璃正在开裂！':current&&current.on&&Math.abs(current.x-170)<=23?'货物到中央：放平梁，保持 0.5 秒入库。':s.msg;$('msg').classList.toggle('rescue',!!s.rescue);
 ...
 s.bricks.forEach(function(b){var spec=cargoTypes[b.type];c.fillStyle=spec.color;c.strokeStyle='#30253a';c.lineWidth=2;if(spec.name==='圆筒'){c.beginPath();c.arc(b.x,b.y-12,13,0,Math.PI*2);c.fill();c.stroke();}else{c.fillRect(b.x-15,b.y-24,30,24);c.strokeRect(b.x-15,b.y-24,30,24);}c.fillStyle='#30253a';c.font='bold 11px sans-serif';c.textAlign='center';c.fillText(spec.name==='重箱'?'重':spec.name==='玻璃箱'?'◇':'↓',b.x,b.y-7);c.font='bold 14px sans-serif';c.fillText('第 '+(s.got+1)+' 件 · '+spec.name,170,25);c.font='12px sans-serif';c.fillText(spec.tip,170,46);if(b.charge>0){c.fillStyle='#408967';c.fillRect(145,375,50*b.charge/.5,5);}});
