...
window.CatchVariation={config:catchConfig,create:function(ctx,config){
 ...
 return {tier:plan.tier,plan:plan,config:cfg,spawn:function(){var i=index++,wave=plan.waves[Math.floor(i/3)%plan.waves.length],r=random(),type=plan.tier===1?['good','good','gold','bad','good','gold'][i%6]:r<cfg.badRate?'bad':r>.83?'gold':'good';return {id:(catchSession()?.runId||'catch')+':'+i,type:type,tutorialRole:plan.tier===1&&type!=='bad'?([0,2,5].includes(i%6)?'A':'B'):null,x:plan.tier===1?[62,278,76,285,264,76][i%6]:(wave.side<0?30:182)+random()*128,vy:cfg.fall*(plan.tier===1?1:.94+random()*.12),vx:plan.tier===1?0:(random()-.5)*(plan.tier-1)*.3,delay:cfg.delay*(plan.tier===1?1:.9+random()*.2)};}};
}};
...
var catchFeedbackRun=null,catchFeedbackSeq=0,catchFeedbackUntil=0,catchFeedbackLastDraw=0;
function catchFeedbackTick(){
 var node=$('ct-event-feedback'),s=window.RoomDirectCore?.catchState();if(!node||!s)return;
 if(s.tier===1&&s.tutorialCaught){var teaching=document.querySelector('.ct-controls>p');if(teaching)teaching.textContent='左接 '+(s.tutorialCaught.A?'✓':'待完成')+' · 右接 '+(s.tutorialCaught.B?'✓':'待完成')+' · 松手减速';}
 var now=performance.now(),resume=catchFeedbackLastDraw&&now-catchFeedbackLastDraw>1500;catchFeedbackLastDraw=now;
 if(catchFeedbackRun!==s.id){catchFeedbackRun=s.id;catchFeedbackSeq=0;catchFeedbackUntil=0;}
 var fresh=(s.events||[]).filter(function(e){return e.runId===s.id&&e.seq>catchFeedbackSeq;});
 if(fresh.length){catchFeedbackSeq=Math.max.apply(null,fresh.map(function(e){return e.seq;}));node.dataset.eventSeq=String(catchFeedbackSeq);
  var e=fresh.findLast(function(e){return e.kind==='hazard';})||fresh[fresh.length-1],name=e.type==='gold'?'牛奶':'花盆';
  node.textContent=e.kind==='hazard'?'碰到碎砖，扣 '+Number(Math.abs(e.timeDelta).toFixed(2))+' 秒':e.kind==='catch'?'接住'+name+'，+'+e.scoreDelta:e.type==='bad'?'避开了碎砖':'漏接'+name+'，调整位置继续接';
  catchFeedbackUntil=now+1700;
  if(!document.hidden&&!resume&&!s.done)try{window.GameFeedback?.emit(e.kind==='hazard'?'error':e.kind==='catch'?'correct':e.type==='bad'?'reveal':'warning',{source:s.id+':catch:'+e.seq});}catch(_){}
 }
 node.hidden=document.hidden||now>catchFeedbackUntil;
}
...
window.catchDirectMount=function(ctx,config){
 ...
 $('app').innerHTML='<section class="catch-mobile '+(themed?'ct-plant':'')+'"><header class="ct-top"><button id="ct-back" aria-label="返回">‹</button><strong>'+(themed?'接住小花盆':'情报空投')+'</strong><span id="count">准备中</span></header><div class="ct-status"><span id="goal">0 / '+config.goal+'</span><span>'+(themed?'花盆 +1 · 牛奶 +2 · 碎砖 −3秒':'绿箱 +1 · 金箱 +2 · 红色避开')+'</span></div><div class="ct-arena"><canvas id="ct-direct-canvas" width="340" height="400"></canvas></div><p id="msg" class="msg" role="status">先看清落物，再和同伴一起准备；按住移动，松手减速。</p><footer class="ct-controls"><div class="ct-role"><b class="ct-role-'+role.toLowerCase()+'">'+role+'</b><span>你负责'+(role==='A'?'左移':'右移')+'，两人共同托住接物布</span></div><button id="ct-ready">我准备好了</button><button id="ct-tap" class="ct-role-'+role.toLowerCase()+'" disabled>'+(role==='A'?'← 按住向左移动':'按住向右移动 →')+'</button><p>松手减速 · 两人共同移动接物布</p></footer></section>';
 var feedback=document.createElement('p');feedback.id='ct-event-feedback';feedback.className='msg';feedback.setAttribute('role','status');feedback.setAttribute('aria-live','polite');feedback.hidden=true;$('msg').after(feedback);
 var roleChip=document.createElement('b');roleChip.className='ct-header-role ct-role-'+role.toLowerCase();roleChip.textContent=role;document.querySelector('.ct-top').insertBefore(roleChip,$('count'));
 $('ct-back').textContent='☰';$('ct-back').setAttribute('aria-label','本局菜单');
 $('ct-back').addEventListener('click',function(e){if(!window.RoomShell)return;e.preventDefault();e.stopImmediatePropagation();RoomShell.menu({help:'你负责'+(role==='A'?'按住向左':'按住向右')+'；松手减速。两人共同移动接物布，花盆 +1、牛奶 +2，碎砖扣 3 秒。',release:function(){var b=$('ct-tap');if(b&&b.onpointerup)b.onpointerup();}});},true);
