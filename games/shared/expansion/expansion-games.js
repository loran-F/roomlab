...
(function (g) {
  ...
  function lookbackEvent(s,type){if(s.mode!=='lookback')return;s.feedbackSeq=(s.feedbackSeq||0)+1;s.feedbackEvents=s.feedbackEvents||[];s.feedbackEvents.push({id:s.id+':lookback:'+s.feedbackSeq,seq:s.feedbackSeq,type,at:s.elapsed,round:s.round,threat:s.threatIndex,completed:s.completed});if(s.feedbackEvents.length>16)s.feedbackEvents.shift();}
  ...
  function shadowEvent(s,type){s.shadowEventSeq=(s.shadowEventSeq||0)+1;s.shadowEvents=s.shadowEvents||[];s.shadowEvents.push({id:s.id+':shadow:'+s.shadowEventSeq,seq:s.shadowEventSeq,type,at:s.elapsed,round:s.round});if(s.shadowEvents.length>24)s.shadowEvents.shift();}
  function shadowWindows(s){const out=[],tape=s.tape||[];for(let i=0;i<tape.length;i++){const a=tape[i],end=i+1<tape.length?tape[i+1].t:(s.phase==='record'?s.recordTime:s.recordLength);if(a.x!==s.layout.button||end<=a.t)continue;const prev=out[out.length-1];if(prev&&Math.abs(prev.end-a.t)<.001)prev.end=end;else out.push({start:a.t,end});}return out;}
  function shadowDiagnosis(s){if(s.phase!=='escape'||s.done)return '';if(s.ghost!==s.exit)return '回声没有停在集合出口：A 重录时最后走到出口并停住。';if(!s.keyCollected)return s.shadowBlocked?'门已关闭，B 尚未取到钥匙：提前出发或请 A 延长踩钮。':'B 尚未取到钥匙：回放开始后先取钥匙，留意开门区间。';if(s.positions.B!==s.exit)return s.shadowBlocked&&!s.layout.keyLatches?'返回时门已关闭：A 要把踩钮时间覆盖往返，再重录。':'钥匙已到手，但 B 尚未集合：返回标记出口。';return '';}
  function shadowRound(s){
    s.shadowPlan=s.shadowPlan||{start:1,end:5};s.shadowBlocked=null;s.shadowWarned=null;s.shadowClosing=null;s.positions={A:0,B:0};s.tape=[];s.ghost=0;s.doorOpen=false;s.keyCollected=false;s.exit=3;s.recordTime=0;s.playTime=0;s.recordLength=s.layout.recordLength;s.moves={A:-1,B:-1};s.phase='plan';s.phaseTime=0;s.attempts=s.attempts||0;s.round++;
  }
  ...
  function input(s,who,d){
    ...
    if(s.mode==='lookback'){
      ...
      if(who==='B'&&d.key==='lure'&&s.phase==='danger'&&!s.rescue&&s.lures>0){s.lures--;s.rescue=true;s.bad=0;message(s,'干扰灯已生效：保护到本次威胁结束，不延长时间；下一次威胁仍需重新应对。');lookbackEvent(s,'rescue');}
      ...
      if(who==='B'&&['planEarlier','planLater','planLonger','planShorter'].includes(d.key)&&['plan','record','waiting'].includes(s.phase)){const p=s.shadowPlan,len=s.recordLength;if(d.key==='planEarlier'&&p.start>0){p.start--;p.end--;}if(d.key==='planLater'&&p.end<len){p.start++;p.end++;}if(d.key==='planLonger')p.end=Math.min(len,p.end+1);if(d.key==='planShorter')p.end=Math.max(p.start+1,p.end-1);message(s,'B 标记希望开门 '+p.start+'—'+p.end+' 秒；需要 A 实际踩钮录下。');return true;}
      if(who==='A'&&d.key==='record'&&['plan','waiting','retry','replay','escape'].includes(s.phase)){shadowRound(s);s.phase='record';s.attempts++;shadowEvent(s,'record');message(s,'A 踩按钮留足通行时间，再走到门禁前的集合出口等待。');}
      if(who==='B'&&d.key==='replay'&&['waiting','retry','escape'].includes(s.phase)&&s.tape.length){s.phase='replay';s.phaseTime=0;s.playTime=0;s.shadowWarned=null;s.shadowClosing=null;shadowEvent(s,'replay');s.shadowBlocked=null;s.positions.B=0;s.ghost=s.tape[0].x;s.keyCollected=false;s.doorOpen=s.ghost===s.layout.button;s.round++;message(s,'B 趁门禁打开去拿钥匙，再回集合出口与 A 会合。');}
      if(['left','right'].includes(d.key)&&s.elapsed-s.moves[who]>=.12){
        ...
        if(allowed){
          ...
          if(who==='B'&&((s.positions.B===3&&next===4)||(s.positions.B===4&&next===3))&&!s.doorOpen){if(!s.shadowBlocked||s.playTime-s.shadowBlocked.at>.7)shadowEvent(s,'blocked');s.shadowBlocked={at:s.playTime,position:s.positions.B};message(s,'第 '+s.playTime.toFixed(1)+' 秒门未开：等回声踩钮，再过门。');return true;}
          s.positions[who]=next;if(who==='B')s.shadowBlocked=null;
          ...
          if(who==='B'&&next===6&&!s.keyCollected){s.keyCollected=true;shadowEvent(s,'key');s.doorOpen=s.layout.keyLatches||s.ghost===s.layout.button;message(s,s.layout.keyLatches?'钥匙已开门，回出口集合！A、B 都到集合出口才算成功。':'钥匙到手！门只在回声踩住按钮时打开，趁现在返回集合出口。');}
          shadowExit(s);
        }
      }
    }
  ...
  function step(s,dt){
    ...
    if(s.mode==='lookback'){
      ...
      if(s.phase==='travel'){
        ...
        if(s.travel>=s.travelLimit){s.phase='warning';s.phaseTime=0;message(s,'监控发现动静！B 快告诉 A 怎么应对。');lookbackEvent(s,'warning');}
      }else if(s.phase==='warning'&&s.phaseTime>=s.warningLimit){s.phase='danger';s.phaseTime=0;s.bad=0;lookbackEvent(s,'danger');}
        ...
        if(s.bad>(s.tier===1?1.2:.85)){damage(s,'本次应对没有及时跟上');if(!s.done)message(s,'本段未通过，已完成的 '+s.completed+' 段保留；稍后继续下一次尝试。');lookbackEvent(s,'hit');s.phase='safe';s.phaseTime=0;}
        else if(s.phaseTime>=s.dangerLimit){if(s.threatIndex+1<s.threats.length){selectThreat(s,s.threatIndex+1);s.phase='warning';s.phaseTime=0;message(s,'下一段预告：重新核对监控，准备切换动作。');lookbackEvent(s,'warning');}else{s.completed++;message(s,'安全通过第 '+s.completed+' 段走廊。');lookbackEvent(s,'safe');s.phase='safe';s.phaseTime=0;}}
      ...
      if(s.phase==='record'){
        ...
        if(s.recordTime>=s.recordLength){s.phase='waiting';s.doorOpen=false;shadowEvent(s,'recorded');message(s,'录影完成。B 播放后拿钥匙再返回集合出口；A 也可以重录。');}
        ...
        const closing=shadowWindows(s).find(w=>w.start<=s.playTime&&w.end>s.playTime&&w.end<s.recordLength);s.shadowClosing=closing&&!(s.layout.keyLatches&&s.keyCollected)?Math.max(0,closing.end-s.playTime):null;if(s.shadowClosing!==null&&s.shadowClosing<=.7&&s.shadowWarned!==closing.end){s.shadowWarned=closing.end;shadowEvent(s,'closing');}
        if(s.playTime>=s.recordLength){s.phase='escape';s.shadowClosing=null;shadowEvent(s,'ended');message(s,s.ghost===s.exit?'A 的回声已在出口等候，B 拿钥匙后回来集合。':'A 的回声没有停在出口。请重录：踩按钮后走到集合出口等待。');}
        shadowExit(s);
      }
    }
  ...
  function snapshot(s,role){
    ...
    if(s.mode==='lookback'){
      Object.assign(v,{feedbackEvents:(s.feedbackEvents||[]).map(e=>({...e})),threatIndex:s.threatIndex,threatCount:s.threats.length,travel:s.travel,travelLimit:s.travelLimit,warningLimit:s.warningLimit,dangerLimit:s.dangerLimit,action:s.action,rescue:s.rescue,openDoor:s.openDoor,lures:s.lures});
      ...
      Object.assign(v,{layout:{...s.layout},positions:{...s.positions},ghost:s.ghost,doorOpen:s.doorOpen,keyCollected:s.keyCollected,exit:s.exit,recordTime:s.recordTime,playTime:s.playTime,recordLength:s.recordLength,attempts:s.attempts,shadowEventSeq:s.shadowEventSeq||0,shadowEvents:(s.shadowEvents||[]).map(e=>({...e})),shadowClosing:s.shadowClosing,shadowPlan:{...s.shadowPlan},shadowBlocked:s.shadowBlocked?{...s.shadowBlocked}:null,tape:s.tape.map(p=>({...p}))});
    }
    return v;
  }
...
function shadowSVG(s){const x=n=>s.mirror?336-shadowTrack[n]:shadowTrack[n],gate=s.mirror?125:211,exit=x(3),bp=x(s.button),kx=x(6);const person=(n,runner=false)=>{const px=x(n),py=runner?202:146,echo=!runner&&s.echo,label=runner?'B':echo?'回声 A':'A',stroke=echo?'#f4e8ff':'#281f32',fill=runner?'#ffa650':echo?'#aa8dbb':'#54adc5';return `<g class="person" data-character="${runner?'B':'A'}" transform="translate(${px} ${py})"><circle cy="0" r="8" fill="${echo?'#baa3ce':'#ffcd9e'}" stroke="${stroke}" stroke-width="2" ${echo?'stroke-dasharray="3 2"':''}/><rect x="-11" y="9" width="22" height="30" rx="6" fill="${fill}" stroke="${stroke}" stroke-width="2" ${echo?'stroke-dasharray="4 2"':''}/><text x="0" y="29" fill="${echo?'#291e35':'#fff'}" text-anchor="middle" font-size="13" font-weight="bold">${runner?'B':echo?'▶':'A'}</text><text x="0" y="-16" text-anchor="middle" font-size="12" fill="#fff3dc" font-weight="bold">${echo?label:""}</text></g>`};return `<svg viewBox="0 0 336 280" aria-label="${s.mirror?'向左':'向右'}出发；集合出口在门禁入口侧"><rect x="1.5" y="1.5" width="333" height="277" rx="11" fill="#b39cb9" stroke="#281f32" stroke-width="3"/><path d="M3 3h330v190H3z" fill="#776087"/><path d="M3 193h330" stroke="#281f32" stroke-width="3"/><g stroke="#886d98" stroke-width="1"><path d="M3 48h330M3 96h330M3 144h330M48 3v190M96 3v190M144 3v190M192 3v190M240 3v190M288 3v190"/></g><rect x="${exit-21}" y="107" width="42" height="155" rx="8" fill="#b2d2bc" fill-opacity=".35" stroke="#d4edd6" stroke-width="2" stroke-dasharray="5 4"/><path d="M${exit} 52v51" stroke="#d4edd6" stroke-width="2" stroke-dasharray="3 4"/><rect x="${exit-38}" y="25" width="76" height="25" rx="5" fill="#d7e8c8" stroke="#281f32" stroke-width="2"/><text x="${exit}" y="42" text-anchor="middle" font-size="14" fill="#294b3a" font-weight="bold">集合出口</text><path d="M${exit-8} 260h16m-8 0v-10m-4 4 4-4 4 4" stroke="#e8f6dc" stroke-width="2" fill="none"/><path d="M${bp} 210H${gate}" stroke="${s.a===s.button&&s.phase!=='plan'?'#ffda65':'#736178'}" stroke-width="5"/><rect x="${gate-14}" y="105" width="28" height="149" rx="4" fill="#3e304a" stroke="#281f32" stroke-width="3"/><rect x="${gate-10}" y="109" width="20" height="${s.open?9:141}" fill="#54adc5"/><path d="M${gate-10} 113h20" stroke="#317e97" stroke-width="3"/><rect x="${gate-24}" y="72" width="48" height="24" rx="4" fill="${s.open?'#b7ddb9':'#fff0c5'}" stroke="#281f32" stroke-width="2"/><text x="${gate}" y="89" text-anchor="middle" font-size="14" fill="#281f32">门禁</text><rect x="${bp-14}" y="${s.a===s.button&&s.phase!=='plan'?199:193}" width="28" height="${s.a===s.button&&s.phase!=='plan'?7:13}" rx="4" fill="${s.a===s.button&&s.phase!=='plan'?'#9dcca5':'#ffda65'}" stroke="#281f32" stroke-width="2"/><text x="${bp}" y="271" text-anchor="middle" font-size="13" fill="#35273e">按钮</text><g opacity="${s.key?.3:1}" transform="translate(${kx} 159)"><circle cx="0" cy="0" r="8" fill="#ffda65" stroke="#281f32" stroke-width="2"/><circle cx="0" cy="0" r="3" fill="#776087"/><path d="M0 8v17h7m-7-6h5" stroke="#281f32" stroke-width="7" fill="none"/><path d="M0 8v17h7m-7-6h5" stroke="#ffda65" stroke-width="4" fill="none"/></g><text x="${Math.max(42,Math.min(294,kx))}" y="271" text-anchor="middle" font-size="13" fill="#35273e">${s.key?'已取钥匙':'钥匙'}</text>${person(s.a)}${person(s.b,true)}</svg>`}
  let callerTab='rules',callerFocus=0,callerRound='',resultConnection=null;const retiredRuns=new Set();
  ...
  function shell(mode,role){
    ...
    const root=document.querySelector('.x-shell');root.classList.add('x-interaction');root.dataset.role=role;
    if(mode!=='shadow'){const heading=root.querySelector('.x-heading');root.querySelector('.x-top>span').replaceWith(heading);}
    const dock=document.createElement('footer');dock.className='x-operation';root.querySelector('.x-paper').after(dock);dock.append(el('x-hint'),el('x-actions'));
    const feedback=el('x-message');dock.before(feedback);
    const duty=root.querySelector('.x-role');const roleChip=duty.cloneNode(false);roleChip.classList.add('x-role-chip');roleChip.append(duty.querySelector('b').cloneNode(true));root.querySelector('.x-top').insertBefore(roleChip,el('x-time'));duty.hidden=true;
    el('back').textContent='☰';el('back').setAttribute('aria-label','本局菜单');
    el('back').onclick=()=>{if(window.RoomShell){RoomShell.menu({help:el('x-duty').textContent||m[role.toLowerCase()],release});return;}if(directCtx){directCtx.abort();return;}if(window._dungeon)dungeonAbort();else{netSend({t:'bye'});clearTimers();closePeer();location.search='';}};
    ...
    function finishPractice(){practice=true;el('x-practice').innerHTML='<p class="x-practiced">✓ 练习完成。正式行动要听搭档的情报。</p>';el('x-ready').disabled=false;el('x-ready').textContent='我准备好了';}
    if(mode==='lookback'){
      let lesson=0,pressAt=null;const area=el('x-practice');
      function lessonPaint(note=''){area.innerHTML='<p>'+(note?note+' · ':'')+(role==='A'?(lesson===0?'练习 1/2：按住前进至少半秒，再松手停步。':'练习 2/2：听到「扫描」时，点躲藏。'):['练习 1/3：红灯亮了，告诉 A 怎么做？','练习 2/3：绿灯亮了，告诉 A 怎么做？','练习 3/3：扫描靠近，告诉 A 怎么做？'][lesson])+'</p>'+(role==='A'?(lesson===0?button('lesson-hold','↑ 按住前进 · 松手停步'):button('lesson-hide','躲藏')):button('lesson-stop','停步')+button('lesson-go','前进')+button('lesson-hide','躲藏'));}
      function startLesson(){if(role==='A'&&lesson===0&&pressAt===null){pressAt=performance.now();area.querySelector('p').textContent='正在前进…保持半秒，再松手停步。';}}
      function releaseLesson(cancelled){if(pressAt===null)return;const elapsed=performance.now()-pressAt;pressAt=null;if(!cancelled&&elapsed>=450){lesson=1;lessonPaint('已停步');}else lessonPaint('再试一次，按住半秒后松手');}
      area.onpointerdown=e=>{if(e.target.closest('[data-x="lesson-hold"]')){e.preventDefault();try{area.setPointerCapture(e.pointerId);}catch(_){}startLesson();}};
      area.onpointerup=()=>releaseLesson(false);area.onpointercancel=area.onlostpointercapture=()=>releaseLesson(true);
      area.onkeydown=e=>{if([' ','Enter'].includes(e.key)&&e.target.closest('[data-x="lesson-hold"]')){e.preventDefault();startLesson();}};
      area.onkeyup=e=>{if([' ','Enter'].includes(e.key)){e.preventDefault();releaseLesson(false);}};
      area.onclick=e=>{const key=e.target.closest('[data-x]')?.dataset.x;if(!key||practice)return;if(role==='A'){if(lesson===1&&key==='lesson-hide')finishPractice();return;}const correct=['lesson-stop','lesson-go','lesson-hide'][lesson];if(key!==correct){lessonPaint('再看一次信号，答错不扣正式机会');return;}lesson++;if(lesson===3)finishPractice();else lessonPaint('答对了');};lessonPaint();
    }else el('x-practice').onclick=e=>{if(e.target.closest('[data-x]'))finishPractice();};
    el('x-ready').onclick=()=>{if(practice)send('ready',role);};
    let heldPointer=null;
    const actionArea=el('x-actions');
    el('x-stage').onclick=e=>{const b=e.target.closest('[data-x]');if(mode==='shadow'&&role==='B'&&b&&b.dataset.x.startsWith('plan'))send(b.dataset.x,role);};
    actionArea.onclick=e=>{const b=e.target.closest('[data-x]');if(!b||b.disabled)return;let k=b.dataset.x;if(mode==='lookback'&&role==='A'){if(k==='go')return;if(k==='hide')k=localAction==='hide'?'stop':'hide';localAction=k;}send(k,role);};
    actionArea.onpointerdown=e=>{const b=e.target.closest('[data-x="go"]');if(mode!=='lookback'||role!=='A'||!b||b.disabled||heldPointer!==null)return;e.preventDefault();heldPointer=e.pointerId;try{actionArea.setPointerCapture(e.pointerId);}catch(_){}localAction='go';send('go',role);};
    function release(){heldPointer=null;localAction='stop';if(mode==='lookback'&&role==='A')send('stop',role);}
    function pointerRelease(e){if(e.pointerId!==heldPointer)return;release();}
    actionArea.onpointerup=actionArea.onpointercancel=actionArea.onlostpointercapture=pointerRelease;
    function key(e){if(/INPUT|TEXTAREA/.test(e.target.tagName))return;if(mode==='lookback'&&role==='A'&&e.code==='ArrowUp'){const b=actionArea.querySelector('[data-x="go"]');if(b&&!b.disabled){e.preventDefault();localAction='go';send('go',role);}return;}const maps=mode==='lookback'?(role==='A'?{Space:'stop',ArrowDown:'hide'}:{ArrowLeft:'left',ArrowRight:'right',Space:'lure'}):mode==='shadow'?{ArrowLeft:'left',ArrowRight:'right'}:{};if(maps[e.code]&&!e.repeat){e.preventDefault();if(maps[e.code]==='stop'){release();return;}const b=actionArea.querySelector('[data-x="'+maps[e.code]+'"]');if(b&&!b.disabled)b.click();}}
    function keyup(e){if(mode==='lookback'&&role==='A'&&e.code==='ArrowUp')release();}
    window.addEventListener('blur',release);document.addEventListener('visibilitychange',release);window.addEventListener('keydown',key);window.addEventListener('keyup',keyup);
    ...
    dispose=()=>{clearInterval(heart);window.removeEventListener('blur',release);document.removeEventListener('visibilitychange',release);window.removeEventListener('keydown',key);window.removeEventListener('keyup',keyup);if(conn&&conn.off)conn.off('close',disconnect);};
  }
  ...
  let lookbackFeedbackRun=null,lookbackFeedbackSeq=0;
  let shadowFeedbackRun='',shadowFeedbackSeq=0;
  function consumeShadowFeedback(s){if(s.mode!=='shadow')return;if(shadowFeedbackRun!==s.id){shadowFeedbackRun=s.id;shadowFeedbackSeq=0;}let chosen=null;for(const e of s.shadowEvents||[]){if(e.seq<=shadowFeedbackSeq)continue;shadowFeedbackSeq=e.seq;if(document.hidden||s.done||s.elapsed-e.at>1)continue;chosen=e;}if(chosen){const sounds={record:'tap',recorded:'correct',replay:'reveal',closing:'warning',blocked:'error',key:'correct',ended:'tick'};g.GameFeedback?.emit(sounds[chosen.type],{source:chosen.id});}}
  function consumeLookbackFeedback(s){if(s.mode!=='lookback')return;if(lookbackFeedbackRun!==s.id){lookbackFeedbackRun=s.id;lookbackFeedbackSeq=0;}for(const e of s.feedbackEvents||[]){if(e.seq<=lookbackFeedbackSeq)continue;lookbackFeedbackSeq=e.seq;if(document.hidden||s.elapsed-e.at>1||s.done)continue;const sound={warning:'warning',danger:'tick',rescue:'reveal',hit:'error',safe:'correct'}[e.type];if(sound)try{g.GameFeedback?.emit(sound,{source:e.id});}catch(_){}}}
  function draw(s,role){
    if(!el('x-stage'))return;
    view=s;API.view=s;consumeLookbackFeedback(s);consumeShadowFeedback(s);const wait=!s.ready.A||!s.ready.B||s.cd>0;
    const isTheme=themed(s)&&s.mode!=='shadow',root=document.querySelector('.x-shell');root.dataset.roomTheme=isTheme?s.roomId:'';root.dataset.gameMode=s.mode;root.dataset.preparing=String(!s.ready.A||!s.ready.B);root.dataset.waiting=String(wait);if(!wait&&!root.dataset.started){root.dataset.started="yes";window.scrollTo(0,0);}
    ...
    if(s.mode==='lookback'){
      const danger=s.phase==='danger',warn=s.phase==='warning',protectedNow=danger&&s.rescue;
      ...
      if(role==='A'){
        html='<div class="x-corridor '+(danger&&!protectedNow?'danger':'')+'"><div class="x-door"><span>'+(protectedNow?'本次威胁已被干扰':s.openDoor===s.door?'通道已就绪':'告诉 B 开哪扇门')+'</span><strong>'+(protectedNow?'干扰灯保护中':s.door==='left'?'← 左门':'右门 →')+'</strong><i class="'+(s.openDoor===s.door?'open':'')+'">'+(protectedNow?'保护至本次威胁结束':s.openDoor===s.door?'门已打开':'等待 B 开门')+'</i></div><div class="x-agent '+s.action+'"><span>A</span></div><div class="x-cupboard">躲藏柜</div><div class="x-floor"></div></div>';
        actions(button('go','↑ 按住前进','primary')+button('hide','躲藏 / 起身'));
        hint=warn?'身后有动静！听 B 报怪物，再选择动作。':danger?'怪物经过！保持 B 指定的动作。':s.phase==='safe'?'暂时安全，准备下一段。':'把门上方向报给 B，开门后按住前进，松手停步。';
      }else{
        const m=s.decoy?['过期录像','前进','这是录像干扰，不要跟着录像停步']:s.paired?['双影同时出现','躲藏','双影巡查，统一躲进掩体']:monsters[s.monster];
        html='<div class="x-combo-step">第 '+(s.threatIndex+1)+' / '+s.threatCount+' 段 · 每段先听预告</div><div class="x-monitor '+(danger&&!protectedNow?'danger':'')+'"><div class="x-monitor-label">后方监控 · '+(protectedNow?'保护已生效':warn?'发现目标':danger?'接近中':'巡查中')+'</div><div class="x-monster '+s.monster+'"><i></i><i></i><b></b></div><h2>'+(protectedNow?'干扰灯保护中':warn||danger?m[0]:'走廊暂时安全')+'</h2><p>'+(protectedNow?'当前威胁：'+m[0]+' · 本段已保护，等待结束':warn||danger?m[2]+' · 喊「'+m[1]+'」':'听 A 报方向，打开对应的门')+'</p></div>';
        actions(button('left','← 开左门')+button('right','开右门 →')+(s.config.lures?button('lure',s.rescue&&danger?'干扰灯保护中':!s.lures?'干扰灯已用完':'干扰灯 · 剩 '+s.lures+' 次','wide'):''));
      ...
      const rescueHint=danger&&s.rescue?'干扰灯保护中 · 还剩 '+Math.max(0,s.dangerLimit-s.phaseTime).toFixed(1)+' 秒，本次威胁结束即失效':role==='B'?(!s.lures?'干扰灯已用完，继续报信配合。':danger?'现在可开干扰灯，保护到本次威胁结束。':warn?'预警中暂不可用；威胁到达后可开干扰灯。':'干扰灯仅在威胁到达时可用。'):'';
      if(rescueHint)hint=rescueHint;
      ...
      if(role==='A'){
        ...
        const roundKey=s.id+':'+s.round;if(callerRound!==roundKey){callerRound=roundKey;callerFocus=0;callerTab='rules';}const p=s.roster[callerFocus]||s.roster[0];
      ...
      if(role==='B'){
        const split=html.indexOf('<div class="x-file-title">');
        if(split>=0)html='<div class="caller-tabs" role="tablist" aria-label="核验资料"><button role="tab" data-caller-tab="rules" aria-selected="'+(callerTab==='rules')+'">今日规则</button><button role="tab" data-caller-tab="roster" aria-selected="'+(callerTab==='roster')+'">人员档案</button></div><div role="tabpanel">'+(callerTab==='rules'?html.slice(0,split):html.slice(split))+'</div>';
      }
      ...
      const windows=shadowWindows(s),windowText=windows.length?windows.map(w=>w.start.toFixed(1)+'—'+w.end.toFixed(1)+'秒').join('、'):'尚未录到踩钮';
      const plan=s.shadowPlan||{start:1,end:5};const planning='<div class="shadow-clock">B 路线：出生点 → 门 → 钥匙 → 集合出口<br>希望开门 '+plan.start+'—'+plan.end+' 秒（计划，不会自动执行）</div>';
      const recorded=s.tape.length?'<div class="shadow-clock">已录开门：'+windowText+'</div>':'';
      const planButtons=role==='B'&&['plan','record','waiting'].includes(s.phase)?button('planEarlier','区间提前 1 秒')+button('planLater','区间推后 1 秒')+button('planLonger','开门多 1 秒')+button('planShorter','开门少 1 秒'):'';
      const check=(yes,text)=>'<span class="'+(yes?'yes':'no')+'">'+(yes?'✓':'○')+' '+text+'</span>';
      html='<div class="shadow-body"><h2>'+title+'</h2><div class="shadow-approved-scene">'+shadowSVG({phase:s.phase,button:layout.button,mirror:layout.mirror,a:actor,b:s.positions.B,echo,open:s.doorOpen,key:s.keyCollected})+'</div><div class="shadow-status">'+status+'</div><div class="shadow-clock">'+timeline+'</div><div class="shadow-checklist">'+check(s.keyCollected,'钥匙')+check(aAt,'A 回声到达')+check(bAt,'B 到达')+'</div>'+planning+recorded+planButtons+'</div>';
      actions(button('left','← 向左走')+button('right','向右走 →')+(role==='A'?button('record',s.done?(won?'共同撤离成功':'本局已结束'):recording?'录制中 · '+Math.ceil(s.recordLength-s.recordTime)+'秒后结束':s.attempts?'重录 · 重置本次回放':'开始录影 · '+s.recordLength+' 秒','wide primary'):button('replay',s.done?(won?'共同撤离成功':'本局已结束'):s.phase==='escape'?'再放一次影子':'播放影子 · 出发','wide primary')));
      ...
      if(s.shadowBlocked)hint=s.message;if(shadowDiagnosis(s))hint=shadowDiagnosis(s);if(s.shadowClosing!==null&&s.shadowClosing!==undefined&&s.shadowClosing<=.7)hint='门将在 '+s.shadowClosing.toFixed(1)+' 秒后关闭，抓紧过门！';
    }
    ...
    if(isTheme){
      ...
      if(s.mode==='lookback'){
        ...
        // Lookback lesson owns its current prompt; render must not reset it.
      }
    }
    ...
    stage.querySelectorAll('[data-caller-tab]').forEach(b=>{b.onclick=()=>{callerTab=b.dataset.callerTab;draw(view,role);el('x-stage').querySelector('[data-caller-tab="'+callerTab+'"]').focus({preventScroll:true});};});
    ...
    if(s.mode==='caller'&&s.phase==='call'&&s.votes[role])hint=s.votes.A&&s.votes.B&&s.votes.A!==s.votes.B?'意见不同，交换证据后可改票。':'你已投'+(s.votes[role]==='admit'?'放行':'拒绝')+' · 等待同伴，可改票';
    ...
    if(isTheme){const duties={caller:['你守门禁：按 B 指定的 '+s.config.checks+' 项核验身份，交换证据后投票。','你查人事档案：报核验项、查员工编号与任务，留意封闭通道。'],shadow:['姐姐 A：录下踩按钮、走到集合出口等待的轨迹。','妹妹 B：播放回声，穿门取钥匙后回出口与姐姐集合。'],lookback:['你看前路：向 B 报门向；听信号选择前进、停步或躲藏。','你看观察台：红灯喊停、绿灯喊走、扫描喊躲；按 A 报的方向开门。']};el('x-duty').textContent=duties[s.mode][role==='A'?0:1];root.querySelector('#x-brief>p').textContent=el('x-duty').textContent;}
    document.querySelectorAll('#x-actions [data-x]').forEach(b=>{
      ...
      if(s.mode==='shadow'){if(k==='left'||k==='right')b.disabled=b.disabled||(role==='A'?s.phase!=='record':!['replay','escape'].includes(s.phase));if(k==='record')b.disabled=b.disabled||!['plan','waiting','retry','replay','escape'].includes(s.phase);if(k==='replay')b.disabled=b.disabled||!['waiting','retry','escape'].includes(s.phase)||!s.tape.length;}
  ...
  function beginResult(s,role){
    ...
    if(!window._dungeon)g.RoomResults.begin({runId:s.id,role,connection:PEER.conn,gameId:s.mode,title:META[s.mode].name,...(s.mode==='shadow'?{ruleset:'shadow-plan-v2',pendingCandidate:true}:{}),dungeon:null,onContinue:()=>{if(role==='A')host(s.mode);},onExit:()=>{netSend({t:'bye'});clearTimers();closePeer();renderHall();}});
  ...
  install();
  g.RoomGameAdapters=g.RoomGameAdapters||{};IDS.forEach(mode=>{g.RoomGameAdapters[mode]={host:ctx=>host(mode,ctx),guest:ctx=>guest(mode,ctx),...(mode==='shadow'?{ruleset:'shadow-plan-v2'}:{})};});
