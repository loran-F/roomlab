...
function config(tier){tier=Math.max(1,Math.min(5,Math.floor(Number(tier)||1)));var i=tier-1;return {tier:tier,tutorial:tier===1,goal:[1,2,3,4,5][i],maxHp:[5,4,3,3,2][i],safeHalf:[8,6,5,5,5][i],accel:[8,9,10,10,10][i],maxSpeed:[4,5,6,6,6][i],brake:28,damping:[8,7,6,6,6][i],jitter:[0,.12,.2,.2,.2][i],stable:[24,24,24,22,20][i],warning:3,moving:1.5,grace:[.8,.6,.5,.5,.5][i],hold:[2,2.5,3,3,3][i],timeLimit:[120,180,240,300,360][i]};}
...
function step(s,dt){
 ...
 // A must release adjustment and let the pressure settle before calibration counts.
 s.steadyFor=s.phase==='stable'&&!s.adjust.up&&!s.adjust.down&&Math.abs(s.velocity)<=.25?(s.steadyFor||0)+dt:0;
 s.steady=s.steadyFor>=.4;
 ...
 if(open&&inside&&s.steady){s.charge+=dt;s.unsafe=0;s.feedback='valid';}
 else if(open&&inside){s.unsafe=0;s.feedback='unsettled';s.charge=Math.max(0,s.charge-dt*.25);}
 ...
 s.risk=s.heatB>=75?'heat':s.unsafe>c.grace?'outside':null;
 if(s.unsafe>=c.grace+1||s.heatB>=100){s.damageReason=s.heatB>=100?'heat':'outside';s.hp--;s.cooldown=1.5;s.requireRelease=true;s.unsafe=0;s.charge=Math.max(0,s.charge-1);s.feedback='damage';rtMessage(s,(s.damageReason==='heat'?'阀门过热':'持续偏离安全区')+'：耐久 −1。松阀冷却，再听 A 报停稳。','err');}
...
pressureSnapshot=function(s,who){var v=baseSnapshot(s,who);v.tier=s.cfg.tier;v.goal=s.cfg.goal;v.maxHp=s.cfg.maxHp;v.grace=s.cfg.grace;v.risk=s.risk||null;v.damageReason=s.damageReason||null;if(who==='A')v.steady=!!s.steady;v.revision=s.revision||0;return v;};
pressureUI=function(who){baseUI(who);};
pressureDraw=function(s,who){baseDraw(s,who);$('rt-health').textContent='设备耐久 '+s.hp+' / '+s.maxHp;$('rt-progress').textContent='校准 '+s.seals+' / '+s.goal;$('rt-help').textContent=who==='A'?'按住加 / 减调节，提前松手停稳。向 B 报压力和趋势；保持不动 0.4 秒后报“停稳”，再由 B 开阀。':'报安全区间，听 A 报“停稳”后按住阀门 '+s.holdGoal+' 秒。移动中不累计；阀温达到 75% 提前松阀。每轮结束先松阀，迁移后重新报区间。';
 var running=s.ready&&s.cd<=0&&!s.done;
 if(running){
  var phase=s.phase==='settle'?'本轮完成 · 松阀':s.phase==='warning'?'迁移预告 · 暂不开阀':s.phase==='moving'?'区间迁移 · 等待':s.cooldown>0?'恢复 · 松阀冷却':s.requireRelease?'等待松阀':s.feedback==='valid'?'稳定开阀':s.feedback==='unsettled'?'压力未停稳 · 松阀等待':'调整 · 报读数';
  $('rt-signal').textContent=phase;
  if(who==='A'&&s.phase==='stable')$('rt-detail').textContent+=(s.steady?' · 已停稳，请报 B':' · 松手等待停稳');
  if(s.risk)$('rt-self').textContent+=' · '+(s.risk==='heat'?'阀温偏高，松阀降温':'偏离过久，立即松阀');
  if(who==='B')$('rt-action').textContent=s.cooldown>0?'松阀冷却中':s.requireRelease?'先松开阀门':s.phase!=='stable'?'等待区间稳定':s.feedback==='unsettled'?'松阀 · 等 A 停稳':'按住开阀';
 }
};
...
function start(who,ctx){clearTimers();cleanup();context=ctx;connection=PEER.conn;role=who;lastRevision=-1;var m=meta(ctx),id=m.runId||ctx.runId||ctx.sessionId;S.mod='pressure';S.role=who;window._dead=false;realtimeUI('pressure',who);var kicker=document.querySelector('.rt-kicker');if(kicker)kicker.textContent=({1:'入门',3:'普通',5:'困难'})[m.tier||ctx.tier]+' · '+config(m.tier||ctx.tier).goal+' 轮合作校准';$('back').textContent='菜单';$('back').setAttribute('aria-label','对局菜单');$('back').onclick=function(){if(window.RoomShell){RoomShell.menu({help:$('rt-help').textContent,release:function(){window.dispatchEvent(new Event('blur'));}});return;}if(confirm('退出本局？')){var c=context;cleanup();if(c&&c.isActive())c.abort();}};function menuGuard(e){if(window.RoomShell&&RoomShell.isOpen())e.stopImmediatePropagation();}window.addEventListener('keydown',menuGuard);var controlsDispose=rtControls(who,'pressure');dispose=function(){window.removeEventListener('keydown',menuGuard);controlsDispose();};window._rtCleanup=cleanup;
...
window.RoomGameAdapters=window.RoomGameAdapters||{};RoomGameAdapters.pressure={ruleset:'pressure-b6-v1',host:function(ctx){start('A',ctx);},guest:function(ctx){start('B',ctx);}};
