...
var locating=false, locationReady={A:false,B:false}, locationNotes=[], localPosition=null;
var scenario=null, serial=0, acceptedEntries=new WeakMap(),directContext=null,directRunning=false,directSeen=null,directPrepared={A:false,B:false};
...
function publicScenario(s){return {schema:2,visibility:'observer',mapId:s.mapId,sessionId:s.sessionId,generation:s.generation,initialTime:s.initialTime,loadingAttempt:s.loadingAttempt||null,rules:JSON.parse(JSON.stringify(s.rules)),map:{walls:s.map.walls.slice(),end:s.map.end.slice(),traps:s.map.traps.map(function(p){return p.slice();}),gatePoints:s.map.gatePoints.map(function(p){return p.slice();}),location:s.map.location?JSON.parse(JSON.stringify(s.map.location)):null}};}
function generate(input){
 var rules=typeof input==='object'?config(input.tier,input.theme):config(input||1),best=null,seed=271828;var random=rules.tier===1&&rules.theme!=='mountain'?function(){seed=(Math.imul(seed,1664525)+1013904223)>>>0;return seed/4294967296;}:Math.random;
...
function valid(s){
 ...
 if(s.visibility==='observer'){
  ...
  if(Object.keys(m).some(function(k){return !['walls','end','traps','gatePoints','location'].includes(k);}))return false;
 }
 ...
 if(rules.theme==='mountain'&&(!window.MazeLocationModel||!MazeLocationModel.validate(m)))return false;
 if(s.visibility==='observer')return true;
 if(m.location&&!m.location.sites.some(function(p){return String(p)===String(m.start);}))return false;
...
window.mazePrepareHost=function(){
 ...
 if(theme==='mountain'){var location=null;for(var tries=0;tries<80&&!location;tries++){if(tries)m=generate(rules);location=MazeLocationModel.construct(m,rules.tier);}if(!location)throw new Error('未能生成安全定位开局，请重试');m.location=location;m.start=location.sites[Math.floor(Math.random()*location.sites.length)].slice();}
...
window.mazeDrawLocal=function(ctx,cell,pos,gates){
 var size=500; if(ctx.canvas.width!==size||ctx.canvas.height!==size){ctx.canvas.width=size;ctx.canvas.height=size;}cell=100;ctx.clearRect(0,0,size,size);localPosition=pos.slice();
 for(var y=0;y<5;y++)for(var x=0;x<5;x++){var r=pos[0]+y-2,c=pos[1]+x-2,visible=Math.abs(y-2)+Math.abs(x-2)<=(locating?1:runtimeRules().visionRadius),wall=!MAZE.walls[r]||MAZE.walls[r][c]!=='0';ctx.fillStyle=!visible?'#1b3147':wall?'#4c667e':'#e7f1f8';ctx.fillRect(x*cell,y*cell,cell,cell);ctx.strokeStyle=visible?'#91aabd':'#263e54';ctx.strokeRect(x*cell+.5,y*cell+.5,cell-1,cell-1);}
 gates.forEach(function(g){if(Math.abs(g.r-pos[0])+Math.abs(g.c-pos[1])<=(locating?1:runtimeRules().visionRadius)){ctx.fillStyle=g.open?'#087b70':'#315ce7';ctx.font='42px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(g.open?'✓':symbols[g.symbol],(g.c-pos[1]+2.5)*cell,(g.r-pos[0]+2.5)*cell);}});drawLocationMarks(ctx,cell,pos);updateObservation();marker(ctx,200,200,cell,'A','#315ce7');var progress=document.getElementById('mz-progress');if(progress)progress.textContent=(mountain()?'保护点':'阀门')+' '+gates.filter(function(g){return g.open;}).length+' / '+runtimeRules().gateCount;
};
drawMazeFull=function(ctx,cell){if(S.role==='B'&&mountain()&&window.MazeArtSkin&&MazeArtSkin.drawFull(ctx,cell,{theme:'mountain',map:MAZE,notes:locationNotes}))return;terrain(ctx,cell,null);drawLocationMarks(ctx,cell,null);MAZE.gatePoints.forEach(function(p,i){marker(ctx,p[1]*cell,p[0]*cell,cell,String(i+1),'#5067ba');});var e=MAZE.end;marker(ctx,e[1]*cell,e[0]*cell,cell,'E','#12786e');MAZE.traps.forEach(function(t){var x=(t[1]+.5)*cell,y=(t[0]+.5)*cell;ctx.fillStyle='#c33e59';ctx.beginPath();ctx.moveTo(x,y-cell*.3);ctx.lineTo(x+cell*.3,y);ctx.lineTo(x,y+cell*.3);ctx.lineTo(x-cell*.3,y);ctx.closePath();ctx.fill();ctx.fillStyle='#fff';ctx.font='bold '+Math.floor(cell*.45)+'px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText('!',x,y);});};

function drawLocationMarks(ctx,cell,pos){
 if(!MAZE.location)return;
 MAZE.location.landmarks.forEach(function(l){if(pos&&Math.abs(l.at[0]-pos[0])+Math.abs(l.at[1]-pos[1])>(locating?1:runtimeRules().visionRadius))return;var x=pos?l.at[1]-pos[1]+2:l.at[1],y=pos?l.at[0]-pos[0]+2:l.at[0];ctx.fillStyle='#274b67';ctx.textAlign='center';ctx.textBaseline='middle';ctx.font='600 '+Math.floor(cell*(pos?.22:.30))+'px sans-serif';ctx.fillText(l.name,(x+.5)*cell,(y+.5)*cell);});
 if(!pos)MAZE.location.sites.forEach(function(p,i){ctx.strokeStyle=locationNotes[i]===2?'#923247':'#315ce7';ctx.lineWidth=locationNotes[i]===1?3:1;ctx.strokeRect(p[1]*cell+2,p[0]*cell+2,cell-4,cell-4);ctx.fillStyle=ctx.strokeStyle;ctx.font='700 '+Math.floor(cell*.3)+'px sans-serif';ctx.textAlign='left';ctx.textBaseline='top';ctx.fillText(String(i+1)+(locationNotes[i]===2?'×':''),p[1]*cell+2,p[0]*cell+2);});
}
function candidateDescription(index){var p=MAZE.location.sites[index],obs=MazeLocationModel.observation(MAZE,p),labels=MAZE.location.landmarks.filter(function(l){var distance=Math.abs(l.at[0]-p[0])+Math.abs(l.at[1]-p[1]);return distance>0&&distance<=2;}).map(function(l){var dr=l.at[0]-p[0],dc=l.at[1]-p[1];return (dr?(dr<0?'上':'下')+Math.abs(dr)+'格':'')+(dc?(dc<0?'左':'右')+Math.abs(dc)+'格':'')+'：'+l.name;});return '查阅石台 '+(index+1)+'（公共地图信息）\n'+MazeLocationModel.dirs.map(function(d,i){return d[2]+'：'+obs[i+1];}).join(' · ')+'\n附近地标：'+(labels.join('；')||'两步内无其他地标');}
function updateObservation(){var el=$('mz-observation');if(!el||!localPosition||!MAZE.location)return;var obs=MazeLocationModel.observation(MAZE,localPosition);el.textContent='脚下：'+obs[0]+'；'+MazeLocationModel.dirs.map(function(d,i){return d[2]+'：'+obs[i+1];}).join(' · ');}
function refreshLocationUI(){var box=$('mz-location');if(!box)return;box.querySelector('h3').textContent=locating?'先共同定位':'位置已核对 · 继续探索';box.querySelectorAll('[data-location-action]').forEach(function(b){b.disabled=b.hasAttribute('data-candidate')?false:!locating;});var status=$('mz-location-status');if(status)status.textContent=locating?'点击石台查看地标：依次标记候选、排除、撤销。笔记不判正误。':'按地图寻找全部保护点，再前往 E。定位为双方确认，不代表系统判定正确。';var explore=document.querySelector('.mz-explore-controls');if(explore&&!locating)explore.hidden=false;var toggle=$('mz-toggle-guide');if(toggle)toggle.setAttribute('aria-expanded',explore&&!explore.hidden?'true':'false');if(!locating&&localPosition&&$('stage')){var canvas=$('stage').querySelector('canvas');if(canvas)window.mazeDrawLocal(canvas.getContext('2d'),100,localPosition,window._mazeGates||[]);}if(!locating){box.querySelectorAll('.mz-location-actions:not(.mz-candidates)').forEach(function(e){e.hidden=true;});var caption=document.querySelector('.mz-map-caption');if(caption&&localPosition)caption.firstElementChild.textContent='A 是你 · '+(runtimeRules().visionRadius===2?'周围两步范围':'周围五格');var hint=$('mz-probe-hint');if(hint)hint.textContent='已结束开局核对；仍可自由探索，迷路时与 B 重新商量。';}updateObservation();}
function settleLocation(){if(!locating)return;if(locationReady.A&&locationReady.B){locating=false;directContext.send({t:'m5Explore',id:directContext.runId||directContext.sessionId});refreshLocationUI();}else{var status=$('mz-location-status');if(status)status.textContent='已确认，等待同伴核对位置。';}}
function mountLocationUI(manual){
 if(!locating)return;var box=document.createElement('section');box.id='mz-location';box.className='mz-controls';box.innerHTML='<h3>先共同定位</h3><p id="mz-location-status"></p>'+(manual?'<p>石台编号是候选位置。听 A 报通道与地标，建议短探查，再记录判断。</p><div class="mz-location-actions mz-candidates"></div><p id="mz-candidate-description" aria-live="polite">点击任一石台，查阅它周围的通道和地标。这里不显示 A 的位置。</p><div class="mz-location-actions mz-probes"></div>':'<p id="mz-observation"></p><p id="mz-probe-hint">描述周围通道；按 B 建议探查一步，回报新地标。</p>')+'<div class="mz-location-actions"><button id="mz-located" data-location-action>已口头核对位置</button></div>';
 var controls=document.querySelector('.mz-controls');if(manual){controls.classList.add('mz-explore-controls');controls.hidden=true;var toggle=document.createElement('button');toggle.id='mz-toggle-guide';toggle.textContent='工具对照 · 随时查看';toggle.setAttribute('aria-expanded','false');toggle.setAttribute('aria-controls','mz-tool-guide');controls.id='mz-tool-guide';toggle.onclick=function(){if(!directLive()||window.RoomShell&&RoomShell.isOpen())return;controls.hidden=!controls.hidden;toggle.setAttribute('aria-expanded',String(!controls.hidden));};controls.before(toggle);}controls.before(box);if(manual)box.after(toggle);
 if(manual){MAZE.location.sites.forEach(function(p,i){var b=document.createElement('button');b.dataset.locationAction='';b.dataset.candidate=i;b.textContent='石台 '+(i+1)+' · 未标记';b.onclick=function(){if(!directLive()||window.RoomShell&&RoomShell.isOpen())return;$('mz-candidate-description').textContent=candidateDescription(i);locationNotes[i]=((locationNotes[i]||0)+1)%3;b.textContent='石台 '+(i+1)+' · '+['未标记','候选','排除'][locationNotes[i]];b.setAttribute('aria-pressed',locationNotes[i]===1?'true':'false');var cv=$('maze-map');drawMazeFull(cv.getContext('2d'),32);};box.querySelector('.mz-candidates').append(b);});MazeLocationModel.dirs.forEach(function(d,i){var b=document.createElement('button');b.dataset.locationAction='';b.dataset.probe=i;b.textContent='探查'+d[2];b.onclick=function(){if(directLive()&&locating&&!(window.RoomShell&&RoomShell.isOpen()))directContext.send({t:'m5Probe',id:directContext.runId||directContext.sessionId,direction:i});};box.querySelector('.mz-probes').append(b);});}
 $('mz-located').onclick=function(){if(!directLive()||!locating||window.RoomShell&&RoomShell.isOpen())return;this.disabled=true;this.textContent='已确认，等同伴';locationReady[manual?'B':'A']=true;if(manual)directContext.send({t:'m5Located',id:directContext.runId||directContext.sessionId});else settleLocation();};refreshLocationUI();
}

function polish(manual){
 if(observer)observer.disconnect();
 var rules=runtimeRules(),tx=rules.text,view=!locating&&rules.visionRadius===2?'周围两步范围':'周围五格';
 ...
 app.classList.add('maze-mobile');app.classList.toggle('has-round-menu',!!window.RoomShell);app.dataset.mazeRole=manual?'B':'A';app.dataset.mazeTheme=mountain()?'mountain':'';
 if(!card||!top)return;
 top.replaceChildren();if(back){back.textContent='‹';back.setAttribute('aria-label',window._dungeon?'返回地图':'返回玩法');top.append(back);}var title=document.createElement('strong');title.textContent=mountain()?'山径迷踪':'通风潜行';top.append(title);var role=document.createElement('span');role.className='mz-role';role.textContent=manual?'B · 指挥':mountain()?'A · 登山':'A · 潜行';top.append(role);var hud=document.createElement('div');hud.className='mz-hud';var clock=document.createElement('span');clock.append('剩余 ');if(count){clock.append(count);if(manual&&scenario&&Number.isFinite(scenario.initialTime))count.textContent=scenario.initialTime+'s';}var progress=document.createElement('span');if(!manual)progress.id='mz-progress';progress.textContent=(mountain()?'保护点':'阀门')+(manual?' · 共 '+rules.gateCount+' 处':' 0 / '+rules.gateCount);hud.append(clock,progress);top.after(hud);
 ...
 var stage=card.querySelector('.stage');stage.setAttribute('aria-label',manual?'本局完整地图':view+'局部视野');var caption=document.createElement('div');caption.className='mz-map-caption';caption.innerHTML=manual?'<span>E '+tx.end+(rules.trapCount?'　◆ '+tx.hazard:'　无危险区')+'</span><span>'+Array.from({length:rules.gateCount},function(_,i){return i+1;}).join(' / ')+' '+(mountain()?'保护点':'阀门')+'</span>':'<span>A 是你 · '+view+'</span><span>向 B 描述附近标记</span>';stage.after(caption);
 ...
 else{var dp=$('dpad'),gate=$('maze-valve');if(dp){var names={up:'↑ 上',l:'← 左',d:'↓ 下',r:'右 →'};dp.querySelectorAll('button').forEach(function(b){b.textContent=names[b.dataset.d];b.setAttribute('aria-label',names[b.dataset.d]);});controls.append(dp);}if(gate)controls.append(gate);if(gate){var instruction=gate.querySelector('p');if(instruction)instruction.textContent='先报出当前标记，再按 B 的指示选择。';gate.setAttribute('aria-label','处理当前'+tx.gate);}
...
var run=runMaze;runMaze=function(set){mazeTerminal=null;mazeStartedAt=performance.now();mazeContext=directContext||(window._dungeon?window._roomGameContext:null);run(set);if(runtimeRules().tier===1)(window._mazeGates||[]).forEach(function(g,i){g.symbol=i%6;});if(directContext){var scope=directContext,move=window._mazeMove;window._mazeMove=function(dr,dc){if(scope.isActive()&&!(window.RoomShell&&RoomShell.isOpen()))return move(dr,dc);};document.querySelectorAll('#dpad button,[data-valve]').forEach(function(b){var click=b.onclick;b.onclick=function(e){if(scope.isActive()&&!(window.RoomShell&&RoomShell.isOpen())&&click)return click.call(this,e);};});}polish(false);if(window._dungeon&&window._netMode==='host')netSend({t:'dgT',time:S.time});};
...
function directPrepareScreen(){
 var tx=runtimeRules().text,role=directContext.role,app=$('app');app.className='maze-mobile';app.dataset.mazeRole=role;app.dataset.mazePhase='ready';app.innerHTML='<header class="topbar"><button id="back" aria-label="对局菜单">菜单</button><strong>'+ (mountain()?'山径迷踪':'通风潜行')+'</strong><span class="mz-role">'+role+'</span></header><section class="mz-controls"><h3>'+ (role==='A'?'观察现场，听同伴指路':'查看地图，为同伴指路')+'</h3><p>'+ (role==='A'?'用方向键移动；遇到'+tx.gate+'，描述标记后选择同伴指定的工具。':'地图不显示 A 的位置，先听 A 描述周围通道。通过工具对照指导 A 处理机关。')+'</p><p>完成 '+runtimeRules().gateCount+' 处'+tx.gate+'后前往'+tx.end+'。双方准备后开始计时。'+(mountain()?'先描述石台周围的通道，短探查后共同定位，再前往机关。':'')+'</p><button id="mz-prepare">我准备好了</button><p id="mz-ready-status">等待双方准备</p></section>';
 $('back').onclick=function(){if(window.RoomShell)RoomShell.menu({help:app.querySelector('.mz-controls').innerText});else if(confirm('退出本局？'))directContext.abort();};
 $('mz-prepare').onclick=function(){directPrepared[role]=true;this.disabled=true;this.textContent='已准备';if(role==='B')directContext.send({t:'m5Prepared',id:directContext.runId||directContext.sessionId});directLaunch();};
}
function directLaunch(){if(!directLive())return;if(directContext.role==='A'&&directPrepared.A&&directPrepared.B&&!directRunning){directRunning=true;directContext.send({t:'m5Go',id:directContext.runId||directContext.sessionId});directShell(false);}else if($('mz-ready-status'))$('mz-ready-status').textContent=directPrepared.A||directPrepared.B?'等待同伴准备':'等待双方准备';}
function directShell(manual){
 locating=!!scenario.map.location;locationReady={A:false,B:false};locationNotes=[];localPosition=null;
 var app=$('app');app.className='';delete app.dataset.mazePhase;app.innerHTML='<div class="topbar"><button id="back">‹</button><span id="count" class="count">'+scenario.initialTime+'s</span></div>'+(manual?'<div class="manual-card"><div class="stage"><canvas id="maze-map"></canvas></div><div id="msg"></div></div>':'<div id="playarea" class="panel"><div id="stage" class="stage"></div><div id="msg"></div><div class="dpad" id="dpad"><button data-d="up">上</button><button data-d="l">左</button><button data-d="d">下</button><button data-d="r">右</button></div></div>');
 ...
 mountLocationUI(manual);
 $('back').textContent='菜单';$('back').setAttribute('aria-label','对局菜单');$('back').onclick=function(){if(!directLive())return;if(window.RoomShell)RoomShell.menu({help:document.querySelector('.mz-help p').textContent});else if(confirm('退出本局？'))directContext.abort();};
}
function directPacket(d){
 ...
 if(d.t==='m5Offer'&&directContext.role==='B'){
  if(directSeen!==d.id){if(!d.scenario||d.scenario.sessionId!==d.id||d.scenario.mapId!==d.id||!d.scenario.rules||d.scenario.rules.tier!==directContext.tier||d.scenario.rules.theme!==(directContext.roomId==='room-16'?'mountain':'vent')||!window.mazeApplyScenario(d.scenario))return true;directSeen=d.id;directPrepareScreen();}
  directContext.send({t:'m5Ready',id:d.id});return true;
 }
 if(d.t==='m5Ready'&&directContext.role==='A'){if(directSeen!==d.id){directSeen=d.id;directPrepareScreen();}return true;}
 if(d.t==='m5Prepared'&&directContext.role==='A'){directPrepared.B=true;directLaunch();return true;}
 if(d.t==='m5Go'&&directContext.role==='B'&&directPrepared.B&&!directRunning){directRunning=true;directShell(true);return true;}
 if(d.t==='m5Located'&&directContext.role==='A'&&directRunning&&locating){locationReady.B=true;settleLocation();return true;}
 if(d.t==='m5Explore'&&directContext.role==='B'&&directRunning&&locating){locating=false;refreshLocationUI();return true;}
 if(d.t==='m5Probe'&&directContext.role==='A'&&directRunning&&locating&&Number.isInteger(d.direction)&&d.direction>=0&&d.direction<4){var hint=$('mz-probe-hint');if(hint)hint.textContent='B 建议：向'+MazeLocationModel.dirs[d.direction][2]+'探查一步，回报新地标。';return true;}
...
function directStart(role,ctx){
 clearTimers();directContext=ctx;directRunning=false;directSeen=null;directPrepared={A:false,B:false};mazeTerminal=null;S.mod='maze';S.role=role;window._dead=false;
 if(role==='A'){var sc=mazePrepareHost(),last=performance.now();$('app').innerHTML='<div class="msg">等待同伴接收地图…</div>';var tick=setInterval(function(){if(!directLive()){clearInterval(tick);return;}if(!directRunning){last=performance.now();if(!directSeen)ctx.send({t:'m5Offer',id:sc.sessionId,scenario:sc});return;}if(mazeTerminal){clearInterval(tick);return;}var now=performance.now(),seconds=Math.floor((now-last)/1000);if(seconds){last+=seconds*1000;S.time=Math.max(0,S.time-seconds);if($('count'))$('count').textContent=S.time+'s';}ctx.send({t:'m5Clock',id:sc.sessionId,time:S.time});if(S.time<=0)finish(false);},150);timers.push(tick);}
...
var directClear=clearTimers;clearTimers=function(){if(directContext){window._dead=true;window._mazeMove=null;directContext=null;directRunning=false;directSeen=null;locating=false;localPosition=null;mazeTerminal=null;mazeContext=null;if(observer)observer.disconnect();}return directClear.apply(this,arguments);};
