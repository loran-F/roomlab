...
function report(data){
 ...
 const rating=s.pendingCandidate?null:g.CooperationRating?.score({gameId:s.gameId,win:data.win,stage,reasonCode,metrics:metrics(data.metrics)},s.ratingConfig||{});
 s.result=Object.freeze({ruleset:s.ruleset,pendingCandidate:!!s.pendingCandidate,rating:rating||null,resultId:s.runId+':result',gameId:s.gameId,win:data.win,stage,reasonCode,reason:typeof data.reason==='string'?data.reason.slice(0,160):reasonText[reasonCode]||'',metrics:Object.freeze(metrics(data.metrics)),dungeon:d&&Object.freeze(d)});
 try{if(!s.pendingCandidate)g.CooperationRating?.record(s.result,s.result.rating,{role:s.role,excluded:s.ratingExcluded});}catch(_){}
...
function consume(p,connection){
 ...
 if(p.t==='roomResult'&&s.role==='B'){
  const r=p.result;if(s.pendingCandidate&&(!r||r.ruleset!==s.ruleset||r.pendingCandidate!==true))return true;if(!r||r.resultId!==s.runId+':result'||r.gameId!==s.gameId||typeof r.win!=='boolean'||!['game-success','game-fail','room-complete','room-fail'].includes(r.stage))return true;
 ...
 }else if(p.t==='roomResultAck'&&s.role==='A'&&s.result&&p.resultId===s.result.resultId){const fresh=!s.acked;s.acked=true;clearInterval(s.retry);if(fresh){const root=document.getElementById('room-result');if(root){root.querySelector('.rr-note').textContent='两端已同步本次结果。';root.querySelector('.rr-primary').disabled=false;const other=root.querySelector('#direct-other-room');if(other)other.disabled=false;}}}
 else if(p.t==='roomResultAdvance'&&s.role==='B'&&s.result&&p.resultId===s.result.resultId&&!s.advanced){s.advanced=true;remove();if(s.onContinue)s.onContinue(s.result,p.action==='retry'?'retry':'select');}
 return true;
}
function advance(action='select'){const s=session;if(!s||s.role!=='A'||!s.result||!s.acked||s.advanced||s.disconnected)return false;s.advanced=true;send(s,packet(s,'roomResultAdvance',{resultId:s.result.resultId,action:action==='retry'?'retry':'select'}));remove();if(s.onContinue)s.onContinue(s.result,action==='retry'?'retry':'select');return true;}
...
function draw(s){
 ...
 if(s.pendingCandidate)root.append(node('p','rr-reason','测试玩法 · 不计正式成绩与评级'));
 if(!s.disconnected&&!s.pendingCandidate)g.CooperationRating?.decorate(root,r,r.rating);
 ...
 exit.onclick=()=>{if(s.roomSession){s.onExit?.();return;}close();if(s.onExit)s.onExit();};next.disabled=s.role==='B'||!s.acked||s.disconnected;next.onclick=()=>advance();if(s.roomSession){next.id='direct-retry';next.textContent=s.role==='A'?'再来一局':'等待房主继续';next.onclick=()=>advance('retry');const other=node('button','','选择其他密室');other.id='direct-other-room';other.disabled=s.role!=='A'||!s.acked||s.disconnected;other.onclick=()=>advance('select');exit.id='direct-leave';exit.textContent='离开房间';actions.append(next,other,exit);}else actions.append(exit,next);footer.append(actions);root.append(footer);document.body.append(root);(next.disabled?exit:next).focus({preventScroll:true});
