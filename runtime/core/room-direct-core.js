...
function snapshot(s){return {id:s.ctx.runId,tier:s.ctx.tier,roomId:s.ctx.roomId,px:s.px,pv:s.pv,items:(s.items||[]).map(x=>({...x})),...(s.tutorialCaught?{tutorialCaught:{...s.tutorialCaught}}:{}),eventSeq:s.eventSeq||0,events:(s.events||[]).map(x=>({...x})),score:s.score,goal:s.config.goal,time:s.time,ready:{...s.ready},cd:s.cd,done:s.done,win:s.win};}
...
function catchHost(ctx,config){
 const s=screen(ctx,config),generator=CatchVariation.create(ctx,config);Object.assign(s,{px:170,pv:0,score:0,time:config.timeLimit,elapsed:0,next:400,items:[],itemSeq:0,eventSeq:0,events:[],tutorialCaught:ctx.tier===1&&config.tutorialRoles===true?{A:false,B:false}:null,right:false,accumulator:0});let last=performance.now(),broadcast=0;
 ...
 while(s.accumulator>=16&&!s.done){s.accumulator-=16;if(!s.ready.A||!s.ready.B)continue;if(s.cd>0){s.cd=Math.max(0,s.cd-.016);continue;}s.elapsed+=16;s.time=Math.max(0,s.time-.016);s.next-=16;if(s.next<=0&&s.items.length<config.maxActive){const it=generator.spawn(s.score);s.items.push({...it,itemId:ctx.runId+':item:'+(++s.itemSeq),y:-18});s.next=it.delay;}
 ...
 for(let i=s.items.length-1;i>=0;i--){const it=s.items[i];if(s.tutorialCaught&&((it.tutorialRole==='A'&&s.held)||(it.tutorialRole==='B'&&s.right)))it.tutorialInputSeen=true;it.vy+=config.gravityPerTick;it.x=Math.max(4,Math.min(336,it.x+wind+(it.vx||0)));it.y+=it.vy;if(it.y>=360){const distance=Math.abs(it.x-s.px),caught=distance<=config.receiverWidth/2+8,scoreBefore=s.score,timeBefore=s.time;if(caught){if(it.type==='bad')s.time=Math.max(0,s.time-config.badPenalty);else s.score+=it.type==='gold'?2:1;}
 if(s.tutorialCaught&&caught&&it.type!=='bad'&&it.tutorialInputSeen&&['A','B'].includes(it.tutorialRole))s.tutorialCaught[it.tutorialRole]=true;
 s.events.push({runId:ctx.runId,seq:++s.eventSeq,itemId:it.itemId,kind:caught?(it.type==='bad'?'hazard':'catch'):'miss',type:it.type,x:it.x,y:it.y,distance,elapsedMs:s.elapsed,scoreDelta:s.score-scoreBefore,timeDelta:s.time-timeBefore});if(s.events.length>32)s.events.splice(0,s.events.length-32);s.items.splice(i,1);}}
 if(s.score>=config.goal&&(!s.tutorialCaught||s.tutorialCaught.A&&s.tutorialCaught.B))finish(true);else if(s.time<=0)finish(false);}
