...
function start(who,ctx){clearTimers();cleanup();context=ctx;connection=PEER.conn;role=who;lastRevision=-1;var m=meta(ctx),id=m.runId||ctx.runId||ctx.sessionId;S.mod='pressure';S.role=who;window._dead=false;realtimeUI('pressure',who);var kicker=document.querySelector('.rt-kicker');if(kicker)kicker.textContent=({1:'入门',3:'普通',5:'困难'})[m.tier||ctx.tier]+' · '+config(m.tier||ctx.tier).goal+' 轮合作校准';$('back').onclick=function(){var c=context;cleanup();if(c&&c.isActive())c.abort();};dispose=rtControls(who,'pressure');window._rtCleanup=cleanup;
