...
function completed(options){const ids=history(options),out=[];for(const p of plans){if(!ids.includes(p.roomId))break;out.push(p.roomId);}return out;}
...
function isRoomUnlocked(room,options){const p=getPlan(room);return !!p&&(testView(options)||p.order<=completed().length+1);}
