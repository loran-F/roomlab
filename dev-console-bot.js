...
function create({windows,mode,signal,onChange=()=>{}}){
 let role='manual',paused=false,stopped=false,timer=null,anchor=null,nextPull=0,resultWaitUntil=0,actions=0,message='双端手动',gameId=null;
 const clean=[];
 ...
 function stop(reason='本轮托管已停止'){if(stopped)return;stopped=true;clear();clean.splice(0).forEach(fn=>fn());report(reason);}
 ...
 function step(){
  timer=null;if(stopped||paused||role==='manual'||signal.aborted)return;
  try{
 ...
 return Object.freeze({setRole,pause,resume,stop,snapshot});
}
window.DevConsoleBot=Object.freeze({capability,create});
})();
