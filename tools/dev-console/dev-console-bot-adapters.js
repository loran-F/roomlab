...
'use strict';
const guided={
 maze:{A:'向手动 B 描述本端观察，再按对方建议移动、确认定位或执行工具口令。',B:'只操作本端候选笔记、探查建议、定位确认和工具表；不读取 A 的出生点、坐标或默认路线。'},
...
function create(mode,role,h){
 ...
 function visibleCommands(){
  ...
  if(mode==='maze'&&role==='A'){if(d.querySelector('#maze-valve:not([hidden])'))return buttons(Array.from(d.querySelectorAll('#maze-valve [data-valve]')).map((button,i)=>['valve:'+i,'选择'+button.textContent.trim()]));const out=[['key:ArrowUp','向上移动'],['key:ArrowDown','向下移动'],['key:ArrowLeft','向左移动'],['key:ArrowRight','向右移动']];if(!d.querySelector('#mz-located')?.disabled)out.push(['maze:located','确认已口头核对位置']);return buttons(out);}
  if(mode==='maze'&&role==='B'){const out=[];d.querySelectorAll('[data-candidate]').forEach(button=>out.push(['maze:candidate:'+button.dataset.candidate,button.textContent.trim()]));d.querySelectorAll('[data-probe]:not(:disabled)').forEach(button=>out.push(['maze:probe:'+button.dataset.probe,button.textContent.trim()]));if(!d.querySelector('#mz-located')?.disabled)out.push(['maze:located','确认已口头核对位置']);if(d.querySelector('#mz-toggle-guide'))out.push(['maze:guide','展开或收起工具对照']);Array.from(d.querySelectorAll('.mz-valve-grid span')).forEach((span,i)=>out.push(['say-valve:'+i,'告诉 A：'+span.textContent.trim()]));return buttons(out);}
 ...
 function command(id){
  ...
  if(id.startsWith('maze:candidate:'))return h.click('[data-candidate="'+id.slice(15)+'"]');
  if(id.startsWith('maze:probe:'))return h.click('[data-probe="'+id.slice(11)+'"]');
  if(id==='maze:located')return h.click('#mz-located');
  if(id==='maze:guide')return h.click('#mz-toggle-guide');
