...
function configuredVariant(config){return catalog?.plans.find(x=>x.roomId===config.roomId)?.variants?.[config.level]||null;}
function botControls(){
 const r=current,config=r?.config||configuration(),mode=r?.started?r.config.mode:$("dc-mode").value,select=$("dc-control"),state=r?.bot?.snapshot(),gameState=r?.started?readGame(r.windows.A,mode):undefined,variant=configuredVariant(config);
 const supported=value=>variant?.pendingCandidate&&value!=='manual'?{supported:false,kind:'manual',detail:'当前为“'+variant.title+'”候选，尚无可靠陪练答案能力；请双端手动。'}:DevConsoleBot.capability(mode,value,gameState);
...
function capability(){const mode=$('dc-mode').value,config=configuration(),variant=configuredVariant(config);if(towerScope())$('dc-capability').textContent='按所选密室的真实成长规则生成完整路线。每次重开使用新 seed；同题固定 seed 重试尚无统一 UI 接口。';else if(variant?.pendingCandidate)$('dc-capability').textContent=variant.title+'：测试候选不计正式成绩或评级；当前没有可靠陪练答案能力，请双端手动。';else $('dc-capability').textContent=mode==='pwslide'&&Number($('dc-level').value)===1?'固定教学题：密码始终为 1234；“重开固定教学”只重置条片和本轮状态。':mode==='pwslide'?'换题会重置全部条片偏移并生成进阶密码；同题复测尚无可靠接口。':(['lockbox','silhouette','evidence','mirrors'].includes(mode)?'换题会重新创建关卡；部分基础结构或案件题库固定，不保证每次题面不同。':'换题会重新创建关卡；固定地图、规则或结构可能重复。')+' 同题重试尚无统一接口。';updateRetryCopy();botControls();}
...
function gameURL(){const u=new URL('roomlab.html',location.href);u.search='';u.searchParams.set('dcBuild',CONSOLE_BUILD);return u.href;}
...
function endpoint(w,mode){
 ...
 try{const d=w.document,connected=!!w.PEER?.conn?.open,loader=w.RoomLoading?.status(),s=readGame(w,mode);let stage='准备中',ready=false;
  ...
  else if(Array.from(d.querySelectorAll('#ct-ready,#pp-ready,#ps-ready,#ls-ready,#mz-ready,#mz-prepare,#bm-ready,#wm-ready,#mb-ready,#radio-ready,#x-ready,#cf-ready,#rt-ready')).some(b=>!b.hidden&&b.getClientRects().length)){stage='等待准备';ready=false;}
  else if(s?.done||s?.phase==='done')stage='已结束';
  else if(mode==='catch'){ready=!!(s?.ready?.A&&s?.ready?.B&&s.cd===0&&!d.getElementById('ct-tap')?.disabled);stage=ready?'可操作':s?.ready?.A&&s?.ready?.B?'倒计时':'等待准备';}
  else if(mode==='maze'){ready=!!s&&!s.done&&!d.getElementById('mz-prepare')&&!d.getElementById('mz-ready');stage=ready?'可操作':'等待准备';}
...
async function prepareUI(r){
 ...
 for(const role of ['A','B']){check(r);const w=r.windows[role],d=w.document,mode=r.config.mode,panel=d.getElementById('coop-panel');
  ...
  if(mode==='lookback'){
   const holdButton=d.querySelector('#x-practice [data-x="lesson-hold"]');
   if(holdButton&&!holdButton.disabled){await hold(r,w,holdButton);continue;}
   const lessonArea=d.getElementById('x-practice'),lessonText=lessonArea?.innerText||'';
   const lessonKey=role==='B'?(lessonText.includes('红灯')?'lesson-stop':lessonText.includes('绿灯')?'lesson-go':lessonText.includes('扫描')?'lesson-hide':''): 'lesson-hide';
   const lessonButton=lessonKey?lessonArea?.querySelector('[data-x="'+lessonKey+'"]'):null;
   if(lessonButton&&!lessonButton.disabled){lessonButton.click();continue;}
   click(w,'#x-practice [data-x]');
  }else if(['caller','shadow'].includes(mode))click(w,'#x-practice [data-x]');
  if(mode==='pwslide'&&d.getElementById('pg-practice-step'))click(w,'#pg-practice-step');
  if(mode==='pwslide'&&d.getElementById('pg-ready'))click(w,'#pg-ready');
  for(const selector of ['#ct-ready','#pp-ready','#ps-ready','#ls-ready','#mz-ready','#mz-prepare','#bm-ready','#wm-ready','#mb-ready','#radio-ready','#x-ready','#cf-ready','#rt-ready'])click(w,selector);
 }
}
...
async function start(config){
 ...
 try{
  ...
  setPhase('hosting','A 正在创建验收房间');a.RoomSession.lobby();
  await wait(r,()=>a.RoomSession.state().screen==='selector'&&a.RoomSession.state().roomCode&&a.PEER?.peer?.open,'A 自动建房失败，请确认本地 Peer 服务已启动。');a.RoomSession.levels(config.roomId);await wait(r,()=>a.RoomSession.state().roomId===config.roomId,'A 无法切换到指定密室。');r.roomCode=String(a.RoomSession.state().roomCode);r.hostPeer=a.PEER.peer;$('dc-room-code').textContent='房间 '+r.roomCode;
  setPhase('joining','B 正在加入同一房间');b.RoomSession.join();const input=b.document.getElementById('direct-code-input'),submit=b.document.getElementById('direct-join');if(!input||!submit)throw new Error('B 端加入表单不完整');input.value=r.roomCode;input.dispatchEvent(new b.Event('input',{bubbles:true}));submit.click();
  ...
  await wait(r,()=>a.RoomSession.state().screen==='proposal'&&b.RoomSession.state().screen==='proposal'&&b.document.getElementById('direct-accept'),'B 端未收到开局提案。');const acceptedId=a.RoomSession.state().proposalId;
  if(!b.RoomSession.acceptProposal())throw new Error('B 端无法确认提案');r.proposal={acceptedId,resourcesBefore};setPhase('preparing','B 已确认，双方正在按需加载并建立本轮会话');
  ...
  const session=a.RoomSession.current(),pictureGuess=session?.ruleset==='picture-guess-v1',adapterA=pictureGuess?a.PictureGuessAdapter:a.RoomGameAdapters?.[config.mode],adapterB=pictureGuess?b.PictureGuessAdapter:b.RoomGameAdapters?.[config.mode];if(!adapterA?.host||!adapterA?.guest||!adapterB?.guest)throw new Error('按需加载后玩法未注册完整 A/B 适配器');
...
function monitor(r){if(current!==r||r.controller.signal.aborted)return;const e=paintEndpoints(r);
 ...
 const sessionA=r.windows.A.RoomSession?.state?.(),sessionB=r.windows.B.RoomSession?.state?.();
 if(sessionA?.screen==='selector'&&sessionB?.screen==='selector'&&!r.windows.A.RoomSession.current?.()&&!r.windows.B.RoomSession.current?.()){r.bot?.stop('本局已退出，托管已停止');setPhase('ended','双方已返回密室选择，连接仍保留');r.monitor=setTimeout(()=>monitor(r),500);return;}
 r.bot?.syncResult();
...
async function discover(){
 const r=current={id:++serial,controller:new AbortController(),frames:{},windows:{},off:[],busy:true};setPhase('catalog','首次读取当前游戏注册信息');
  try{if(location.protocol==='file:')throw new Error('请通过本地服务打开：http://127.0.0.1:8080/dev-console.html');const w=await load(r,'A',true),plans=w.RoomDirectory.plans;if(plans.length!==17)throw new Error('直达目录应为 17 房，当前为 '+plans.length);const rooms=plans.map(p=>{const room=w.ROOM_CATALOG.find(x=>x.id===p.roomId);return {id:p.roomId,name:room?.name||p.title,mode:p.primary,title:p.title};});catalog={rooms,modes:rooms.map(x=>({id:x.mode,name:x.title,roomId:x.id})),plans:plans.map(p=>({roomId:p.roomId,primary:p.primary,title:p.title,variants:Object.fromEntries([1,2,3].map(level=>[level,w.RoomDirectory.variant(p.roomId,level)]))})),accessMode:'developer',version:w.APP_VER,progressionVersion:2,protocol:3};
