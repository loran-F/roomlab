...
function layout(role){
 ...
 $('app').innerHTML='<section class="beat-mobile"><header class="bt-top"><button id="back" aria-label="返回">‹</button><strong>信号节拍</strong><span id="count">'+count+'</span></header><div class="bt-status"><span id="goal">'+goal+'</span><span>蓝 A · 橙 B · 紫色一起</span></div><div class="bt-arena" id="stage"></div><div id="msg" class="msg" role="status">等音符进入黄色判定区，再点击下方按钮。</div><footer class="bt-controls"><div class="bt-role"><b class="bt-role-'+role.toLowerCase()+'">'+role+'</b><span>你负责'+(role==='A'?'蓝色 A':'橙色 B')+'，紫色 AB 与同伴一起点</span></div><button id="bt-tap" class="bt-role-'+role.toLowerCase()+'" disabled>♫ 点击'+(role==='A'?'蓝色 A':'橙色 B')+'节拍</button><p>点一下判定一次，无需长按</p></footer></section>';
 $('stage').appendChild(canvas);if(fb)$('app').appendChild(fb);$('back').onclick=gobackMenu;
 var button=$('bt-tap');button.onclick=function(){if(!button.disabled&&window._beatPress){window._beatPress(role);button.classList.add('bt-pressed');setTimeout(function(){button.classList.remove('bt-pressed');},120);}};
