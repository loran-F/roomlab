...
function botControls(){
 ...
 select.value=desiredRole;select.disabled=!catalog||!!r?.busy;
 $("dc-bot-capability").textContent=mode==='pwslide'&&!supported('A').supported?supported('A').detail:supported(desiredRole).detail+(mode==="pwslide"?" 首批支持 A/B。":mode==="lightsearch"?" 首批仅支持 A；B 搜索尚未适配。":" 本玩法 A/B 托管均未适配。");
...
const captions={catalog:'读取游戏目录',idle:'待开局',loading:'加载游戏页面',hosting:'创建房间',joining:'加入房间',map:'准备密室素材',preparing:'自动准备',playing:'进行中',ended:'本关已结束',error:'开局或连接失败'};
...
function capability(){const mode=$('dc-mode').value;$('dc-capability').textContent=mode==='pwslide'?'换题会重置全部条片偏移；入门密码固定为 1234，不保证数字变化。同题复测尚无可靠接口，暂不可用。':(['lockbox','silhouette','evidence','mirrors'].includes(mode)?'换题会重新创建关卡；部分基础结构或案件题库固定，不保证每次题面不同。':'换题会重新创建关卡；固定地图、规则或结构可能重复。')+' 同题重试尚无统一接口，暂不可用。';}
function controls(){const busy=!!current?.busy;['dc-room','dc-mode','dc-level'].forEach(id=>$(id).disabled=!catalog||busy);$('dc-start').disabled=busy;$('dc-start').textContent=catalog?'一键开局':'重新读取目录';$('dc-retry').disabled=!lastConfig||busy;$('dc-back').disabled=!catalog&&!current;$('dc-retry-mode').disabled=busy;botControls();}
...
function cancel(){const r=current;current=null;if(r)clearFrames(r);for(const role of ['a','b']){$('dc-state-'+role).textContent='未连接';$('dc-state-'+role).dataset.ready='false';}$('dc-room-code').textContent='房间 —';$('dc-error').hidden=true;setPhase('idle','选择配置后开局');}
...
async function load(r,role,discovery=false){
 ...
 check(r);const w=f.contentWindow;r.windows[role]=w;
...
async function prepareUI(r){
 ...
 for(const role of ['A','B']){check(r);const w=r.windows[role],d=w.document,mode=r.config.mode,panel=d.getElementById('coop-panel');
  ...
  if(mode==='dial'&&d.getElementById('radio-ready')?.disabled&&!r.radio.has(w)){r.radio.add(w);w.dispatchEvent(new w.KeyboardEvent('keydown',{key:'ArrowRight',bubbles:true}));w.dispatchEvent(new w.KeyboardEvent('keyup',{key:'ArrowRight',bubbles:true}));}
...
async function start(config){
 ...
 $('dc-room').value=config.roomId;$('dc-mode').value=config.mode;fillLevels();$('dc-level').value=String(config.level);capability();
 ...
 try{
  ...
  if(!a.RoomProgression.supportsDungeon(config.mode))throw new Error('当前版本未提供此玩法的双人密室入口');
...
function configuration(){return {roomId:$('dc-room').value,mode:$('dc-mode').value,level:Number($('dc-level').value)};}
async function discover(){
 const r=current={id:++serial,controller:new AbortController(),frames:{},windows:{},off:[],busy:true};setPhase('catalog','首次读取当前游戏注册信息');
 try{if(location.protocol==='file:')throw new Error('请通过本地服务打开：http://127.0.0.1:8080/dev-console.html');const w=await load(r,'A',true);catalog={rooms:w.ROOM_CATALOG.map(x=>({id:x.id,name:x.name})),modes:w.PLAYGROUNDS.filter(x=>w.RoomProgression.supportsDungeon(x.id)).map(x=>({id:x.id,name:x.name})),plans:w.RoomProgression.plans.map(x=>({roomId:x.roomId,primary:x.primary})),version:w.APP_VER};
  $('dc-room').replaceChildren(...catalog.rooms.map(x=>option(x.id,x.name)));$('dc-mode').replaceChildren(...catalog.modes.map(x=>option(x.id,x.name)));$('dc-room').value='room-13';$('dc-mode').value='pwslide';fillLevels();cancel();
 }catch(e){if(e.name!=='AbortError'){fail(r,e);$('dc-start').disabled=false;$('dc-start').textContent='重新读取目录';}}
}
$('dc-room').onchange=()=>{const p=catalog?.plans.find(x=>x.roomId===$('dc-room').value);if(p)$('dc-mode').value=p.primary;fillLevels();};$('dc-mode').onchange=fillLevels;
...
window.DevConsole=Object.freeze({snapshot(){const r=current;return {phase:document.body.dataset.phase,sessionId:r?.id||null,config:r?.config?{...r.config}:null,roomCode:r?.roomCode||null,error:r?.error||null,busy:!!r?.busy,bot:r?.bot?.snapshot()||null,catalog:catalog?JSON.parse(JSON.stringify(catalog)):null,A:endpoint(r?.windows.A,r?.config?.mode),B:endpoint(r?.windows.B,r?.config?.mode)};}});
discover();
})();
